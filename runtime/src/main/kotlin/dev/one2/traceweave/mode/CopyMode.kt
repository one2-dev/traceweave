 ...
 * the next unwind level the copy is recognised by its [Copy.MARKER] frame and the new frame is appended,
...
internal fun insertCopyFrame(
  ...
  val copy = tryCopy(flying)
  if (copy == null) {
    warnOnce(flying) { "traceweave: no copier for ${flying.javaClass.name}; using the original" }
    return flying
  }
  ...
  if (copy.stackTrace.isEmpty()) {
    ...
    warnOnce(flying) {
      "traceweave: copy of ${flying.javaClass.name} came back without a writable stack trace; " +
        "using the original"
    }
    return flying
  }
...
// here in later commits). null means "no copier" only; a copier that *throws* is left to propagate to
// handle()'s best-effort catch, which logs it with its cause and returns the original -- so a failure
// is never swallowed silently here.
